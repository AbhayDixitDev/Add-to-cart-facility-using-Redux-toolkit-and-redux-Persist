          ...
          {items.map(item => (
            <Col key={item.id} className="d-flex">
              <Card style={{flex: '1 1 auto', border: '1px solid lightgray'}}>
                ...
                <Card.Body>
                  <div style={{display: 'flex', justifyContent: 'space-between'}}><Button variant="primary" size="sm" className="d-inline-block align-items-center me-2" onClick={()=>handleAddToCart(item)}><FaShoppingCart className="me-2" />Add to Cart</Button>
                  <Button variant="secondary" size="sm" className="d-inline-block" onClick={()=>handleShowModal(item)}>View</Button></div>
